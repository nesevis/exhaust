...
struct CharacterSetRangeExtractionTests {
    ...
    func characterFromDecimalDigits() {
        let gen = #gen(.character(from: .decimalDigits))
        #exhaust(gen) { char in
    ...
    func characterFromLowercaseLetters() {
        let gen = #gen(.character(from: .lowercaseLetters))
        #exhaust(gen) { char in
    ...
    func stringFromAlphanumerics() {
        let gen = #gen(.string(from: .alphanumerics, length: 1 ... 10))
        #exhaust(gen) { string in
    ...
    func variadicCharacterFromEquivalence() {
        let variadicGen: ReflectiveGenerator<Character> = .character(from: .letters, .decimalDigits)
        let unionGen: ReflectiveGenerator<Character> = .character(from: .letters.union(.decimalDigits))
