...
struct ExamineCoverageTests {
    ...
    func integerDecileCoverage() {
        ...
        let intEntry = report.decilesCovered.first { $0.type == "Int" }
        #expect(intEntry != nil)
        #expect(intEntry!.value >= 7)
    }
    ...
    func unsignedDecileCoverage() {
        ...
        let uintEntry = report.decilesCovered.first { $0.type == "UInt" }
        #expect(uintEntry != nil)
        #expect(uintEntry!.value >= 7)
    }
    ...
    func doubleDecileCoverage() {
        ...
        let doubleEntry = report.decilesCovered.first { $0.type == "Double" }
        #expect(doubleEntry != nil)
        #expect(doubleEntry!.value >= 3)
    }
    ...
    func boolNoDeciles() {
        ...
        #expect(report.decilesCovered.isEmpty)
    }
    ...
    func suppressResolution() {
        ...
        #expect(logsConfig.suppressLogs == true)
        #expect(logsConfig.suppressIssueReporting == false)

        let issueConfig = ExamineReportingConfiguration(from: [.suppress(.issueReporting)])
        #expect(issueConfig.suppressLogs == false)
        #expect(issueConfig.suppressIssueReporting == true)

        let allConfig = ExamineReportingConfiguration(from: [.suppress(.all)])
        #expect(allConfig.suppressLogs == true)
        #expect(allConfig.suppressIssueReporting == true)
    }
    ...
    func assertAllDecilesCoverMinimum() {
        ...
        #expect(report.decilesCovered.allSatisfy { $0.value >= 5 })
    }
}
