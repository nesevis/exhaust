...
struct LargeUnionListShrinkingChallenge {
    /*
     ...
     Given a list of lists of arbitrary sized integers, we want to test the property that there are no more than four distinct integers across all the lists. This is trivially false, and this example is an artificial one to stress test a shrinker's ability to normalize (always produce the same output regardless of starting point).

     In particular, a shrinker cannot hope to normalize this unless it is able to either split or join elements of the larger list. For example, it would have to be able to transform one of [[0, 1, -1, 2, -2]] and [[0], [1], [-1], [2], [-2]] into the other.
     */
    ...
    func largeUnionListPathologicalSingle() {
        let value = [
    ...
    func largeUnionListPathologicalSingle2() {
    ...
    func largeUnionListPathologicalSingle3() {
    ...
    func largeUnionList50() throws {
        ...
        for list in lists {
            ...
            #expect(output?.flatMap(\.self).sorted() == [-2, -1, 0, 1, 2])
        }
    }
}
