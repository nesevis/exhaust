...
struct BoundaryCoverageIntegrationTests {
    ...
    func findsFailureAtBoundary() {
        ...
        let result = #exhaust(gen, .budget(.custom(coverage: 200, sampling: 100, reduction: .fast)), .suppressIssueReporting) { a, b in
            !(a == 0 && b == 10000)
        }
    ...
    func threeParamBoundaryInteraction() {
        ...
        let result = #exhaust(gen, .budget(.custom(coverage: 200, sampling: 10, reduction: .fast)), .suppressIssueReporting) { a, b, _ in
            !(a == 0 && b == 10000)
        }
