...
struct SIMDGeneratorTests {
    ...
    func simd2ExplicitRange() {
        #expect(#examine(.simd2(.float(in: 0 ... 1)), .budget(50)).passed)
    }

    @Test("simd2 per-lane")
    func simd2PerLane() {
        #expect(#examine(.simd2(.float(in: 0 ... 1), .float(in: -1 ... 0)), .budget(50)).passed)
    }

    @Test("simd3 with explicit range")
    func simd3ExplicitRange() {
        #expect(#examine(.simd3(.double(in: -1 ... 1)), .budget(50)).passed)
    }

    @Test("simd4 with explicit range")
    func simd4ExplicitRange() {
        #expect(#examine(.simd4(.float(in: 0 ... 1)), .budget(50)).passed)
    }
    ...
    func simd8ExplicitRange() {
        #expect(#examine(.simd8(.int32(in: 0 ... 255)), .budget(50)).passed)
    }

    @Test("simd16 with explicit range")
    func simd16ExplicitRange() {
        #expect(#examine(.simd16(.uint8(in: 0 ... 127)), .budget(50)).passed)
    }

    @Test("simd32 with explicit range")
    func simd32ExplicitRange() {
        #expect(#examine(.simd32(.uint8(in: 0 ... 127)), .budget(30)).passed)
    }

    @Test("simd64 with explicit range")
    func simd64ExplicitRange() {
        #expect(#examine(.simd64(.uint8(in: 0 ... 127)), .budget(30)).passed)
    }
    ...
    func simd2SizeScaled() {
        #expect(#examine(.simd2(.uint8()), .budget(50)).passed)
    }

    @Test("simd4 with size-scaled int32")
    func simd4SizeScaledInt32() {
        #expect(#examine(.simd4(.int32()), .budget(50)).passed)
    }
}
