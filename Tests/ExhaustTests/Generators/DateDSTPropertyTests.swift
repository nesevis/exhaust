...
struct DateDSTPropertyTests {
    ...
    func googleClosureLibraryRelativeDayLabelBugOnDSTFallBack() {
        ...
        let counterExample = #exhaust(gen, .suppress(.issueReporting), .budget(.extensive)) { now, target in
            var calendar = Calendar(identifier: .gregorian)
            calendar.timeZone = Self.usEastern
    ...
    func dateFormatterReturnsNilForValidCalendarDatesWhereMidnightDoesntExist() {
        ...
        let counterExample = #exhaust(gen, .suppress(.issueReporting), .log(.debug)) { date in
