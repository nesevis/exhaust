...
struct GeneratorTuningIntegrationTests {
    ...
    func pickAdaptationWeightsByPredicate() throws {
        ...
        let values = try #example(gen, count: 200, seed: 123)
    ...
    func tuneOutperformsRawGeneration() throws {
        ...
        let rawValues = try #example(gen, count: 200, seed: 99)
        let rawValidCount = rawValues.count(where: predicate)

        // Tuned filter: all output satisfies the predicate
        let tunedValues = try #example(gen.filter(.probeSampling, predicate), count: 200, seed: 99)
    ...
    func depthBudget() throws {
        ...
        let values = try #example(filtered, count: 20, seed: 123)
    ...
    func bstProbeSamplingOutperformsRawGeneration() throws {
        ...
        let tunedValues = try #example(tunedGen, count: sampleCount, seed: 42)
    ...
    func bstTimedBenchmark() throws {
        ...
        let rejectValues = try #example(rejectGen, count: 10000, seed: 42)
        ...
        let tuneValues = try #example(tuneGen, count: 10000, seed: 42)
    ...
    func bstProbeSamplingNonLeaf() throws {
        ...
        let tunedGen = BST.arbitrary().filter(.probeSampling, isValidNonLeafBST)
        let values = try #example(tunedGen, count: 500, seed: 99)
