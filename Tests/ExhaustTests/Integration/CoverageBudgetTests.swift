...
struct CoverageBudgetTests {
    ...
    func randomOnlySkipsCoverage() {
        ...
        #exhaust(gen, .budget(.custom(coverage: 200, sampling: 50, reduction: .fast)), .randomOnly) { _, _ in
            true
        }
    }
    ...
    func coverageBudgetParsed() {
        ...
        #exhaust(gen, .budget(.custom(coverage: 50, sampling: 50, reduction: .fast))) { _, _, _ in
            true
        }
    }
}
