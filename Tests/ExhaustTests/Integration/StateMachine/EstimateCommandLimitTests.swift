...
@Suite("estimateCommandLimit", .serialized, .tags(.stateMachine))
struct EstimateCommandLimitTests {
    ...
    func parameterFree2CommandPickProducesCeilingCappedResult() {
        ...
        let limit = __ExhaustRuntime.estimateCommandLimit(commandGen: gen, screeningBudget: 200)
    ...
    @Test("Zero screening budget produces exploration floor")
    func zeroScreeningBudgetProducesExplorationFloor() {
        let gen = Gen.pick(choices: [
            (weight: 1, generator: Gen.just("a").map { $0 as Any }),
            (weight: 1, generator: Gen.just("b").map { $0 as Any }),
        ])
        let limit = __ExhaustRuntime.estimateCommandLimit(commandGen: gen, screeningBudget: 0)
