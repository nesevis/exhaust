// MARK: - Key-Value Store Lifecycle StateMachine Test
...
//
// This is the first StateMachine integration test to exercise lifecycle-gated
...
// store is closed (and on `open` when already open) test that the spec
...
@Suite("Key-value store lifecycle spec tests", .serialized, .tags(.stateMachine))
struct KVStoreLifecycleTests {
    @Test("Stale data after close/reopen detected via invariant or postcondition")
    func staleDataAfterClosereopenDetectedViaInvariantOrPostcondition() async throws {
