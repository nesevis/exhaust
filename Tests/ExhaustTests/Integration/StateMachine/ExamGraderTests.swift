// MARK: - Exam Grader StateMachine Test
...
//
// Rather than a standalone property test with a custom generator, this port reframes the problem as a `@StateMachine` (stateful) test. The system under test is a `BuggyExamGrader` that manages exams and submitted answer sheets. Two deliberate bugs are embedded:
//
...
//
// The spec exposes these bugs through two complementary mechanisms:
//
...
@Suite("Exam grader spec tests", .serialized, .tags(.stateMachine))
struct ExamGraderTests {
    /// Runs the spec and verifies that Exhaust detects at least one of the two embedded bugs. With sequence lengths of 3 to 8 commands, the spec reliably triggers either the invariant failure (mismatched answer length) or the postcondition failure (grading penalizes blanks). The test passes when the trace contains a failure — meaning the spec successfully caught the bug.
