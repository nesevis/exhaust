...
private func nestedFixture() throws -> (scope: EncoderInput, pickNodeID: Int) {
    ...
    let bindNodeID = try #require(graph.nodes.indices.first { index in
        if case .bind = graph.nodes[index].kind { return true }
        return false
    })
    guard case let .bind(bindMetadata) = graph.nodes[bindNodeID].kind else { throw FixtureError.shape }
    ...
    guard case let .pick(pickMetadata) = graph.nodes[pick].kind, pickMetadata.selectedID == 1 else { throw FixtureError.shape }
...
private final class LiftCounter: @unchecked Sendable {
    var count = 0
}
