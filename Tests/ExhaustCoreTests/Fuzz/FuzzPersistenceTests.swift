...
struct FuzzPersistenceTests {
    ...
    func versionMismatch() throws {
        ...
        future.version = FuzzProgressDocument.currentVersion + 1
    ...
    func writerOrdering() throws {
        ...
        let writer = FuzzProgressWriter(store: store)
    ...
    func corpusQuarantine() {
        ...
        func breadcrumbSurvivesTrap() throws {
            ...
            let survivor = try #require(FuzzBreadcrumb.readSurvivor(fileURL: crumbURL))
...
private func document(consumedNanoseconds: UInt64, clusterCount: Int) -> FuzzProgressDocument {
    let sequence: ChoiceSequence = [
        .value(ChoiceSequenceValue.Value(choice: ChoiceValue(7, tag: .int64), validRange: nil)),
    ]
    let clusters = (0 ..< clusterCount).map { index in
        FuzzProgressDocument.ClusterRecord(
    ...
    return FuzzProgressDocument(
        ...
        clusters: clusters,
        snapshot: [FuzzProgressDocument.CorpusEntryRecord(entry: entry)]
    )
}
