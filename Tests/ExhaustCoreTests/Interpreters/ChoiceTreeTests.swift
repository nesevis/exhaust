...
struct ChoiceTreeTests {
    ...
    struct ContainsPicks {
        ...
        func branchReturnsTrue() {
            let branch = ChoiceTree.branch(
                fingerprint: 0, weight: 1, id: 0, branchIDs: [0, 1],
                choice: .just
            )
        ...
        func nestedBranchInGroup() {
            let branch = ChoiceTree.branch(
                fingerprint: 0, weight: 1, id: 0, branchIDs: [0],
                choice: .just
            )
    ...
    struct PickComplexityTests {
        ...
        func singleBranch() {
            let branch = ChoiceTree.branch(
                fingerprint: 0, weight: 1, id: 0, branchIDs: [0, 1, 2],
                choice: .just
            )
            // 3 branches * 2^0 = 3
        ...
        func nestedBranches() {
            let inner = ChoiceTree.branch(
                fingerprint: 1, weight: 1, id: 0, branchIDs: [0, 1],
                choice: .just
            )
    ...
    struct BranchIdTests {
        ...
        func extractsFromBranch() {
            let tree = ChoiceTree.branch(
                fingerprint: 0, weight: 1, id: 42, branchIDs: [42],
                choice: .just
            )
        ...
        func extractsFromSelectedBranch() {
            let branch = ChoiceTree.branch(
                fingerprint: 0, weight: 1, id: 7, branchIDs: [7],
                choice: .just
            )
...
private let branchNode = ChoiceTree.branch(
    fingerprint: 0, weight: 1, id: 1, branchIDs: [1, 2],
    choice: .just
)
