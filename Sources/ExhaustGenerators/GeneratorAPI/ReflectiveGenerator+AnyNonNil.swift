...
public extension ReflectiveGenerator {
    ...
    static func anyNonNil(
        ...
        for (weight, generator) in choices {
            precondition(weight > 0, "anyNonNil arm weights must be greater than zero")
        ...
        return Gen.backtrack(
            ...
            onExhausted: {
                reportError(
                    "Every arm of anyNonNil(always:) produced nil, so no value could be generated and the run ended early. Add an arm that cannot fail here, or use anyNonNil(_:) and handle nil.",
                    fileID: fileID,
                    filePath: filePath,
                    line: line,
                    column: column
    ...
    static func anyNonNil(
        ...
        anyNonNil(always: choices, fileID: fileID, filePath: filePath, line: line, column: column)
    }
    ...
    /// ```
    ///
    /// `nil` is the result when every arm produces `nil`, and it is a recorded, reflectable, mutation-reachable outcome: the reducer can move a value toward absence and the mutator can flip a node into or out of it. Use this form when absence is meaningful (an optional subterm, an annotation that may be omitted), or when the node sits inside a larger backtracking generator whose enclosing arms should treat its exhaustion as their own failure. When nil would indicate a generator bug, use ``anyNonNil(always:fileID:filePath:line:column:)`` and avoid the `Optional` plumbing. An arm that produces nil is always a withdrawn arm, never the recorded absence, so `.just(nil)` in this list is dead weight.
    ///
    /// Failed attempts consume randomness but are not recorded. The choice sequence holds only the winning arm, so reduction, mutation, and replay see an ordinary weighted choice with one extra branch for absence, ordered after the real arms so reduction prefers a value over `nil`. A node costs up to the sum of its arms' attempts, not the average. Reflection decomposes a value through the first arm in declaration order that can produce it, exactly as `oneOf` does.
    ///
    /// - Parameter choices: Weighted arms. Weights are relative frequencies and must be positive.
    static func anyNonNil<Wrapped>(
        _ choices: [(Int, ReflectiveGenerator<Wrapped?>)],
        fileID: StaticString = #fileID,
        line: UInt = #line,
        column: UInt = #column
    ...
    static func anyNonNil<Wrapped>(
        ...
        anyNonNil(choices, fileID: fileID, line: line, column: column)
    }
}
