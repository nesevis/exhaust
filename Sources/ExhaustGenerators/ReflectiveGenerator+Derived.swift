...
public extension __Exhaustable.Conformance {
    ...
    /// ```
    ///
    /// With no arguments, this uses the annotation's defaults. It is available when importing `ExhaustGenerators` or `Exhaust`; the application's annotated type only needs `Exhaustable`. Use the returned ``ReflectiveGenerator`` directly with `#example` or compose it with `#gen`.
    ///
    ...
    ///
    /// Other derived types resolve this type structurally; declaring a custom generator property does not change that resolution. Use ``gen(depth:maximumNodes:stateSpace:overriding:)`` to pin the root depth instead of drawing it. An explicit root ceiling overrides the root annotation; nested annotated types retain their own ceilings.
    ///
    /// - Parameters:
