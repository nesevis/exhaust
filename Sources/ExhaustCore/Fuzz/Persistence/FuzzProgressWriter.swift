...
package final class FuzzProgressWriter: @unchecked Sendable {
    // @unchecked: all mutable work is confined to the serial queue.
    private let queue = DispatchQueue(label: "com.exhaust.fuzz.progress-writer", qos: .utility)
    private let store: FuzzProgressStore

    package init(store: FuzzProgressStore) {
        self.store = store
    }
    ...
    package func submit(_ makeDocument: @escaping @Sendable () -> FuzzProgressDocument) {
