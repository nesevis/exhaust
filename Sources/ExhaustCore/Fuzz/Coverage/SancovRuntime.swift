...
package enum SancovRuntime {
    ...
    package static func pcTableHash() -> UInt64 {
        ...
        for region in regions {
            ...
            hash = Xoshiro256.fold(hash, mixing: UInt64(entryCount))
            guard entryCount > 0 else {
                continue
            }
            ...
            for index in 0 ..< region.count {
                ...
                hash = Xoshiro256.fold(hash, mixing: UInt64(entry.programCounter &- base))
                hash = Xoshiro256.fold(hash, mixing: UInt64(entry.flags))
            }
        }
        return hash
    }
