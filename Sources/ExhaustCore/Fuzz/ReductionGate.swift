...
package struct ReductionGate {
    ...
    ///
    /// The per-symptom cap uses *admitted* counts as a proxy for the cluster's reduced count — a symptom does not map one-to-one onto a cluster, so the true per-cluster count is not addressable from here. Symptom matching is exactly the weak signal the inventory distrusts, so a capped symptom still escapes, bounding the risk of a genuinely new bug hiding behind a familiar symptom. A coverage-novel failure escapes immediately (a new fault's first failing input necessarily lights edges nothing else has hit, so novelty is a far sharper new-bug signal than cadence), and the periodic fallback for new bugs on already-covered paths adapts through ``noteEscapeOutcome(symptom:isNewCluster:)``.
