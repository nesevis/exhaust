/// Distribution strategy controlling how tightly generated values cluster
/// around an origin as the generator size grows from 1 to 100.
///
/// Inspired by Hedgehog's `Range` types. The scaling is applied in bit-pattern
/// space by ``Gen/choose(in:scaling:)`` so it works uniformly across all
/// ``BitPatternConvertible`` types.
public enum SizeScaling<Bound: Sendable>: Sendable {
    ...
    /// Tightest at small sizes — ideal for centering on 0 while still reaching
