...
public enum FilterType: Equatable, Hashable, CaseIterable {
    ...
    /// Uses ``choiceGradientSampling`` to tune pick weights via CGS with fitness
    /// sharing, producing the best balance of validity rate and output diversity.
    case auto
    ...
    /// Uses online CGS derivative sampling to condition pick weights on upstream
    /// choices, then bakes them with fitness sharing to prevent overcommitting to
    /// the dominant cluster. Produces the best-quality outputs for recursive
    /// generators (e.g. BSTs, AVL trees) at the cost of a short warmup pass.
