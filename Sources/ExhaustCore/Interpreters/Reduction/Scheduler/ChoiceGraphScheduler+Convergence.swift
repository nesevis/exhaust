...
extension ChoiceGraphScheduler {
    ...
    ///
    /// Each record carries the leaf's current bit pattern at extraction time, or nil for exempt leaves. ``transferConvergence(_:to:)`` uses the bit pattern as an identity proxy: `sequenceChild` path steps are positional sibling indices, so after an index-shifting acceptance (mid-sequence deletion, migration, sibling swap) the same path can address a different logical element in the new graph. A surviving element's value is unchanged by structural acceptances, so a value mismatch at the same path identifies a shifted neighbor rather than the record's owner.
    ///
