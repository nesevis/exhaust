...
extension ReductionState {
    ...
    func runBaseDescent(
    ...
    ) throws -> (dependencyGraph: ChoiceDependencyGraph?, progress: Bool) {
        ...
        while budget > 0 {
            ...
            while budget > 0 {
                let dependencyGraph = rebuildDAGIfNeeded()
                if try runStructuralDeletion(budget: &budget, dependencyGraph: dependencyGraph, scopeRange: scopeRange) {
    ...
    func buildDAG() -> ChoiceDependencyGraph? {
        let cdg = rebuildDAGIfNeeded()

        // Phase 2 validation: compare ChoiceGraph against CDG + BindSpanIndex.
        if isInstrumented, let cdg, let choiceGraph, let bindIndex {
            let result = ChoiceGraphComparison.validate(
                graph: choiceGraph,
                cdg: cdg,
                bindIndex: bindIndex,
                sequence: sequence
            )
            ChoiceGraphComparison.logResult(result)
        }
    ...
    private func runStructuralDeletion(
        ...
        let allScopes = buildDeletionScopes(dependencyGraph: dependencyGraph)
        ...
        if let dependencyGraph {
            let antichainNodes = dependencyGraph.maximalAntichain()
    ...
    private func runJointBindInnerReduction(budget: inout Int, cycle: Int = 0, scopeRange: ClosedRange<Int>? = nil) throws -> Bool {
        ...
        if bindInnerCount <= 3 {
            ...
            let bindDependencyGraph = ChoiceDependencyGraph.build(
                from: sequence, tree: tree, bindIndex: bindSpanIndex
            )
            productSpaceBatchEncoder.bindIndex = bindSpanIndex
            productSpaceBatchEncoder.dependencyGraph = bindDependencyGraph
            productSpaceBatchEncoder.dependentDomains = computeDependentDomains(
                bindSpanIndex: bindSpanIndex, dependencyGraph: bindDependencyGraph
            )
            ...
            let chainContext = ReductionContext(bindIndex: bindSpanIndex, dependencyGraph: bindDependencyGraph, filterValidityRates: filterValiditySnapshot)
            let fullRange = scopeRange ?? (0 ... max(0, sequence.count - 1))
...
extension ReductionState {
    ...
    private func buildDeletionScopes(dependencyGraph: ChoiceDependencyGraph?) -> [DeletionScope] {
        ...
        for nodeIndex in dependencyGraph.topologicalOrder {
            let node = dependencyGraph.nodes[nodeIndex]
            switch node.kind {
        ...
        let hasUncoveredDepthZeroContent = dependencyGraph.leafPositions.contains { leafRange in
