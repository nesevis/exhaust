...
///
/// Composable encoders are role-agnostic probe strategies. Each operates on a scoped position range and produces candidate sequences — it does not know or care whether it is assigned to the upstream role, the downstream role, or the standalone role. The role is determined by where the scheduler places the encoder in the pipeline, not by the encoder itself.
///
/// ## Composability
///
