...
extension ChoiceGraph {
    ...
    // MARK: - General Subtree Extraction
    ...
    static func extractSubtreeAtPath(
        ...
        walkForSubtreeAtPath(tree: tree, remainingPath: path[...])
    }

    private static func walkForSubtreeAtPath(
        tree: ChoiceTree,
        remainingPath: ArraySlice<BindPathStep>
    ) -> ChoiceTree? {
        if remainingPath.isEmpty {
        ...
        case let .sequence(_, elements, _):
            ...
            return walkForSubtreeAtPath(
                tree: elements[index],
                remainingPath: remainingPath.dropFirst()
            )

        case let .branch(_, _, _, _, choice):
            return walkForSubtreeAtPath(tree: choice, remainingPath: remainingPath)
        ...
        case let .bind(_, inner, bound):
            if inner.isGetSize {
                return walkForSubtreeAtPath(tree: bound, remainingPath: remainingPath)
            }
            guard let step = remainingPath.first,
                  case .bindBound = step
            else { return nil }
        ...
        case let .selected(inner):
            return walkForSubtreeAtPath(tree: inner, remainingPath: remainingPath)
        }
    }

    // MARK: - Structural Path
    ...
    func structuralPath(to nodeID: Int) -> BindPath? {
        ...
        while let parentID = nodes[current].parent {
            ...
            case let .sequence(metadata):
                guard let index = metadata.childIndexByNodeID[current] else { return nil }
                steps.append(.sequenceChild(index))
            case .zip:
                guard let index = parent.children.firstIndex(of: current) else { return nil }
                steps.append(.groupChild(index))
            case let .pick(metadata):
                steps.append(.pickBranch(metadata.selectedID))
            case let .bind(metadata):
                if parent.children[metadata.boundChildIndex] == current {
                    steps.append(.bindBound)
                }
