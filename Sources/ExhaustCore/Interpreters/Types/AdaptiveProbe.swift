...
package enum AdaptiveProbe {
    ...
    @inlinable
    @inline(__always)
    package static func findInteger<T: BinaryInteger>(_ predicate: (T) -> Bool) -> T {
    ...
    @inlinable
    @inline(__always)
    package static func binarySearchWithGuess<T: BinaryInteger>(_ predicate: (T) -> Bool, low: T, high: T, guess: T? = nil) -> T {
        let guess = guess ?? low
