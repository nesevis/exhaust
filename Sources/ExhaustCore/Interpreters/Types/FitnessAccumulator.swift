...
///
/// - SeeAlso: ``FilterObservation`` (per-filter-site predicate satisfaction), ``CoOccurrenceMatrix`` (per-direction pairwise membership), ``DirectedExploreRunner`` (per-direction hit tracking). These types accumulate empirical observations about different dimensions of the generator's output space — choice-level fitness, filter validity, and directional coverage — using the same count-based accumulation pattern.
