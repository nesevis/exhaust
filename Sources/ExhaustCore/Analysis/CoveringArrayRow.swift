//
//  CoveringArrayRow.swift
//  Exhaust
//

/// A single row in a covering array, mapping parameter indices to value indices.
package struct CoveringArrayRow: @unchecked Sendable {
    // @unchecked Sendable: explicit annotation for consistency with related analysis types (`EnumerableDomainProfile`, `LargeDomainProfile`). All stored properties are `Sendable`.

    /// `values[i]` is a value index in `0..<parameters[i].domainSize`.
    public var values: [UInt64]

    /// Creates a row with the given value indices.
    public init(values: [UInt64]) {
        self.values = values
    }
}
