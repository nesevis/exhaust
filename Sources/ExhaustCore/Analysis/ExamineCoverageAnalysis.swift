...
package enum ExamineCoverageAnalysis {
    ...
    package static func analyze(trees: [ChoiceTree]) -> ExamineCoverageResult {
        ...
        return ExamineCoverageResult(
            decilesCovered: decileAccumulator.results(),
...
package struct ExamineCoverageResult: Sendable {
    /// Per-TypeTag decile coverage. Each entry reports how many of 10 histogram bins received at least one observation.
    package let decilesCovered: [(type: String, value: Int)]
            ...
            case let .choice(value, metadata):
                ...
                switch tag {
                    ...
                    case .double, .float, .float16:
                        ...
                        deciles.record(tag: tag, normalized: min(max(normalized, 0), 1))
                    ...
                    default:
                        ...
                        deciles.record(tag: tag, normalized: normalized)
                }
...
private struct DecileAccumulator {
    ...
    mutating func record(tag: TypeTag, normalized: Double) {
    ...
    func results() -> [(type: String, value: Int)] {
        buckets.sorted(by: { $0.key < $1.key }).compactMap { discriminator, hits in
            ...
            return (type: name, value: covered)
        }
    }
