...
package struct SprawlPersistenceContext {
    ...
    package let resumeDocument: SprawlProgressDocument?
    ...
    package init(store: SprawlProgressStore, resumeEnabled: Bool) {
        self.store = store
        ...
        resumeDocument = store.load(maxAgeSeconds: SprawlTunables.progressLogStalenessSeconds)
        survivor = resumeDocument == nil
            ? nil
            : SprawlBreadcrumb.readSurvivor(fileURL: store.breadcrumbFileURL)
    }
