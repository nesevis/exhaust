...
package struct SprawlProgressStore: Sendable {
    ...
    package func load(maxAgeSeconds: Double) -> SprawlProgressDocument? {
        guard let data = try? Data(contentsOf: progressFileURL) else {
            return nil
        }
        guard let document = try? JSONDecoder().decode(SprawlProgressDocument.self, from: data) else {
            return nil
        }
        guard document.version == SprawlProgressDocument.currentVersion else {
            return nil
        }
    ...
    package func write(_ document: SprawlProgressDocument) throws {
