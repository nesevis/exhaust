...
/// ```
///
/// Bounded presets limit fixed-width integers and binary floating-point magnitudes to 10, 100, or 10,000 at full size. They also limit automatically derived array lengths, set and dictionary cardinalities, string lengths, and `Data` lengths to 5, 10, or 20. Numeric bounds and sequence lengths grow with size; unsigned bounds start at zero, and numeric bounds are clipped to the destination type's range. `.tiny` and `.small` favor collisions by limiting `Date` to 21 or 201 daily values centered on January 1, 2026 UTC. `.medium` leaves the default `Date` domain unchanged because its width does not increase generation or reflection work. Explicit payload overrides keep their own domains.
///
/// An unconfigured root uses `.full`, preserving each built-in generator's existing domain, including its default sequence maximum of 100 and `Date.distantPast...Date.distantFuture` at one-minute resolution. Nested annotations cap the inherited preset, so a nested `.full` does not widen a parent's `.small`. An explicit factory argument replaces the root annotation, but nested annotations still cap their occurrences. Reflection rejects numeric values and sequence lengths outside the selected size-scaled domain. The underlying date leaf retains its documented behavior of rounding to the preceding grid value and clamping to the selected range; enclosing derived constructors still require the reflected replay to reproduce their complete value.
