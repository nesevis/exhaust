...
protocol PreemptiveBackend<Spec>: Sendable {
    ...
    /// Runs one tagged command sequence concurrently using a pre-computed lane partition, applying the setup step to every spec instance before any command runs.
    ///
    ...
    ///
    /// Called after lane-collapse reduction on oracle-flagged failures. If any valid interleaving produces matching responses and passes the oracle, the execution is linearizable and the failure was a false positive. Every replay instance the search constructs receives the same setup step, or the oracle comparison would be meaningless.
    ///
    /// - Parameters:
    ///   - taggedCommands: The full tagged command sequence (prefix + concurrent).
    ///   - setupStep: The candidate's setup step, applied to every replay instance.
