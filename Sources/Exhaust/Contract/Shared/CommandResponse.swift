...
///
/// Marked `@unchecked Sendable` because the value is consumed immediately by the caller after each command execution and never shared across isolation boundaries. Actor-isolation crossing in async contract runners is the only reason `Sendable` conformance is required.
