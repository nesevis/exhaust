/// The return value of ``ContractSpec/run(_:)`` and ``AsyncContractSpec/run(_:)``: a command's description paired with its return value.
///
...
///
/// Marked `@unchecked Sendable` because the value is consumed immediately by the caller after each command execution and never shared across isolation boundaries. The actor-isolation crossing in async contract runners is the only reason `Sendable` conformance is required.
