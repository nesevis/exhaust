...
///
/// - Note: Synthesized generators are approximately three times slower per iteration than hand-written generators for the same type. The overhead comes from reconstructing each value through `init(from: Decoder)` — the only available construction path for types defined outside the test target — rather than calling the memberwise initializer directly. For hot-path benchmarks or large iteration counts, consider writing a generator by hand.
///
/// ```swift
