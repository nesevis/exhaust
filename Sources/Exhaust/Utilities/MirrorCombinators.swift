...
public extension __ExhaustRuntime {
    ...
    /// ## Why this exists
    ///
    /// The generator argument needs its own type-inference context so that dot-syntax expressions (for example `.int()`) resolve against `ReflectiveGenerator<Input>` rather than the macro's return type `ReflectiveGenerator<Output>`.
    ///
    /// - Parameters:
    ...
    ///
    /// This is **macro infrastructure** for enum case generators. The backward closure uses pattern matching to extract associated values, returning `nil` when the enum value doesn't match the expected case. This enables `pick` to prune non-matching branches during reflection.
    ///
    /// The pattern-matching approach is inspired by [swift-case-paths](https://github.com/pointfreeco/swift-case-paths) by Point-Free, which uses `guard case let` for enum associated value extraction.
    ///
    /// - Parameters:
    ///   - generator: The input generator.
    ...
    /// It must be `public` because macro expansions emit code at the call site (in the user's module), but it is not intended for direct use.
    ///
    /// ## Why this exists
    ///
    /// `Gen.zip(a, b).mapped(forward:backward:)` requires the `backward` closure to return the zip's tuple type (for example `(String, Int)`). The `#gen` macro doesn't have type information — it only knows argument labels — so it can't synthesize typed casts for each tuple element. This function sidesteps the problem by operating entirely at the `[Any]` level: the backward pass uses `Mirror` to extract child values by label into `[Any]`, and the forward pass reconstructs the typed tuple via parameter pack iteration over the `[Any]` array.
    ///
    /// - Parameters:
    ///   - generators: The generators to zip, one per struct/class init parameter.
    ///   - labels: Argument labels from the initializer call, ordered to match generator position. Used by Mirror to extract the corresponding property values in the backward pass.
    ///   - forward: The user's transform closure (for example `{ name, age in Person(name: name, age: age) }`).
    /// - Returns: A bidirectional generator that can be reflected backward via Mirror.
    ...
    ///
    /// This is **macro infrastructure** for enum case generators with multiple associated values. The backward closure uses pattern matching to extract associated values, returning `nil` when the enum value doesn't match.
    ///
    /// The pattern-matching approach is inspired by [swift-case-paths](https://github.com/pointfreeco/swift-case-paths) by Point-Free, which uses `guard case let` for enum associated value extraction.
    ///
    /// - Parameters:
    ...
    /// Forwarding wrapper for `Gen.zip`, used by macro expansion for the no-closure multi-generator overload (for example `#gen(a, b)` with no trailing closure).
