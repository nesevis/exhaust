    ...
    private static func replayWithChoicesHelper<Output>(
        ...
        case let .impure(operation, continuation):
            ...
            case let .contramap(_, subGenerator), let .prune(subGenerator):
                ...
                guard let subResult = try replayWithChoicesHelper(subGenerator, choices: &choices) else {
                    return nil
                }
    ...
    private static func replayRecursive<Output>(
        ...
        case let .impure(operation, continuation):
            ...
            case let .resize(_, nextGen):
                ...
                guard let subResult = try replayRecursive(nextGen, with: firstChoice) else {
                    return nil
                }
            ...
            case let .pick(choices):
                ...
                guard let result = try replayRecursive(chosenGen, with: choice) else {
                    return nil
                }
            ...
            case let .contramap(_, subGenerator):
                ...
                guard let subResult = try replayRecursive(subGenerator, with: script) else {
                    return nil
                }
            ...
            case let .prune(subGenerator):
                ...
                // The choices are consumed by its sub-generator. We pass the same script down.
                guard let result = try replayRecursive(subGenerator, with: script) else {
                    return nil
                }
                return result as? Output

            case let .filter(gen, _, _), let .classify(gen, _, _):
                guard let result = try replayRecursive(gen, with: script) else {
                    return nil
                }
                return result as? Output
            }
