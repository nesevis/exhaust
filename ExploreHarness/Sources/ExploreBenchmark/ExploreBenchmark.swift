...
struct BenchmarkRecord: Codable {
    ...
    init(report: FuzzReport, seed: UInt64, arm: String, fixture: String, budgetSeconds: Double) {
        ...
        screeningAttempts = report.attempts.screening
        samplingAttempts = report.attempts.sampling
        mutationAttempts = report.attempts.mutation
        totalAttempts = report.attempts.total
        discardedAttempts = report.attempts.discardedByMaterializer
        duplicateCandidatesSkipped = report.attempts.duplicatesSkipped
        pruneIdentitySkips = report.invocations.pruneIdentitySkips
        elapsedSeconds = report.timing.elapsed.seconds
        attemptsPerSecond = report.attemptsPerSecond
        overheadFraction = report.timing.testingOverheadFraction
        coveredEdges = report.coverage.coveredEdges
        instrumentedEdges = report.coverage.instrumentedEdges
        edgeSingletons = report.coverage.singletons
        edgeDoubletons = report.coverage.doubletons
        estimatedNextEdgeProbability = report.coverage.estimatedNextEdgeProbability
        estimatedReachableEdges = report.coverage.estimatedReachableEdges
        corpusEntries = report.coverage.corpusEntryCount
