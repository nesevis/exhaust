...
**Infrastructure (implemented).** `ConvergenceSignal` on `ConvergedOrigin` carries typed encoder observations across cycles. `nonMonotoneGap(remainingRange:)` is the signal for non-monotonicity — the factory pattern-matches on it and emits a `LinearScanEncoder` (bounded exhaustive scan, ≤ 64 values) in place of `BinarySearchEncoder` for the flagged coordinate. `EncoderConfiguration` replaces the old `Direction` discriminant, generalizing cache validation to all encoder types. The scan produces `scanComplete(foundLowerFloor:)` so the factory reverts to binary search on the next cycle. The scan-revert-scan loop converges naturally on coupled surfaces.

**Detection (deferred).** Within a single binary search run, every rejection occurs below a known failure — the stepper cannot distinguish "the floor is genuinely at bestAccepted" from "there's a gap in the failure surface below bestAccepted." The original `sawUnexpectedResult: Bool` proposal collapses distinct observations into a single bit and fires on every normal binary search narrowing. Detection approaches for Phase 1b: (1) cross-cycle analysis — compare convergence bounds across cycles with stable neighbors; (2) validation probe extension — probe `floor - 2` after the existing `floor - 1` validation; (3) post-convergence spot check — the factory emits a small `LinearScanEncoder` range below every convergence point where `remainingRange ≤ 16`. See `docs/convergence-signals-design.md` for the full design.

**Response (implemented).** `LinearScanEncoder` scans upward from the range lower bound (or from semantic simplest outward). Stops early when a failure is found. Cost bounded at ≤ 64 probes. If the remaining range exceeds the threshold, the factory re-emits `BinarySearchEncoder` and accepts the non-minimal floor. Joint non-monotonicity from inter-coordinate coupling is redistribution's domain.
...
**Resolved.** The runtime bail gap is closed by `DownstreamPick` — a runtime `pick` over the encoder space that selects exhaustive (≤ 64), pairwise (2–20 params), or ZeroValue (catchall) based on actual fibre characteristics at `start()` time. No `SearchMode` needed — the reduce-vs-discover distinction is implicit in the composition structure (the outer decoder evaluates the composed probe).
...
**Branch 3: convergence transfer wasteful → steps 1 + 2 + 3 + 4/5.** Not triggered. Convergence transfer is 0/0/0 across every test. The transfer mechanism requires a search-based downstream encoder; the current downstream is `FibreCoveringEncoder` or `ZeroValueEncoder` (via `DownstreamPick`), both of which cold-start.

**Branch 4: not measurably wasteful → do nothing.** Steps 1–3 are implemented. The runtime bail gap (compositions doing lifts with 0 downstream probes) is closed by `DownstreamPick` — its ZeroValue alternative catches fibres that `FibreCoveringEncoder` would have bailed on.

### What was built

Steps 1–3 and post-termination verification are implemented. The encoder algebra — 20 monolithic encoders consolidated into 13 composable ones, `EncoderFactory` with descriptor chains replacing the slot rotation, `MorphismDescriptor` with dominance edges replacing hand-coded multi-tier orchestration — is the infrastructure that makes steps 1–3 possible and steps 4–5 feasible. The `ComposableEncoder` protocol, `DownstreamPick` for runtime strategy selection, and the structural-constancy split in the factory's prediction logic are the extension points the closed-loop reducer will use. `SearchMode` was removed after analysis showed the reduce-vs-discover distinction is implicit in the composition structure — no encoder needs to change behavior based on it (reduction morphisms like `BinarySearchEncoder` assume monotonic failure boundaries; discovery-compatible encoders like `FibreCoveringEncoder` and `ZeroValueEncoder` already work without it).

**Convergence signals.** `ConvergenceSignal` on `ConvergedOrigin` carries typed encoder observations across cycles — `monotoneConvergence`, `nonMonotoneGap(remainingRange:)`, `zeroingDependency`, `scanComplete(foundLowerFloor:)`. `EncoderConfiguration` replaces the old `Direction` discriminant, generalizing cache validation to all encoder types. `ZeroValueEncoder` detects batch-fail + individual-accept and produces `zeroingDependency`. The factory suppresses `ZeroValueEncoder` for coordinates with `zeroingDependency` and emits `LinearScanEncoder` for coordinates with `nonMonotoneGap` within the 64-value scan threshold.

**Non-monotonicity detection.** `BinarySearchEncoder` emits `nonMonotoneGap(remainingRange:)` when binary search converges above the target and the remaining range is within the scan threshold (≤ 64 values). The factory emits `LinearScanEncoder` for those coordinates on the next cycle. The scan resolves whether a lower floor exists (non-monotone gap) or the binary search floor is correct — a one-time cost bounded at 64 probes per coordinate. After the scan, `scanComplete(foundLowerFloor:)` tells the factory to revert to binary search. Net result across the test suite: -65 materializations (improvements on Replacement, Parser, LargeUnionListPath1, LengthList; small increases on a few others from scan cost).

**Edge observations.** `FibreSignal` and `EdgeObservation` store per-edge downstream observations keyed by region index. The composition writes `exhaustedClean`, `exhaustedWithFailure`, or `bail(paramCount:)` after each edge completes. Structurally constant edges with `exhaustedClean` at the same upstream value are skipped on subsequent cycles. Edge observations are invalidated on structural change.

**Per-signal profiling.** `profilingSummary` includes `signals=Xdep/Yclean/Zfail/Wbail` — zeroing dependency count, fibre exhausted clean/with-failure/bail counts. Coupling shows `signals=3dep/2clean/0fail/0bail`; CouplingScaling(constant) shows `signals=0dep/1clean/0fail/0bail`.
...
The `AdaptiveStrategy` is implemented alongside `StaticStrategy` via the `SchedulingStrategy` protocol. See `docs/closed-loop-reducer-design.md` for the full design and comparison results. Key outcomes:

- **Phase 1 skip**: skips structural deletion when span extraction and prior-cycle acceptances prove no work exists. BinaryHeap saves 117 materializations (17%) with a 36% wall-clock speedup.
- **Per-edge budget adaptation**: observation-driven sub-budgets for composition edges (productive +50%, clean/bail -50%).
- **Signal-driven Phase 3 gating**: skips exploration when all edges were `exhaustedClean` in the prior cycle.
- **zeroingDependency escalation**: runs redistribution immediately when coupled coordinates are detected.
- **Budget split dropped**: profiling showed no phase hits its budget cap. Unified 2000-materialization ceiling.
- **Phase ordering dropped**: reverse-dependency case not demonstrated. Phase 1 skip addresses the same problem more directly.

Selectable via `.adaptiveScheduling` on `ExhaustSettings`. Same counterexample quality on every test.

### Gaps remaining

1. **Convergence transfer.** Still 0/0/0. Requires a search-based downstream that uses warm-start. `FibreCoveringEncoder` and `ZeroValueEncoder` always cold-start. Steps 4+5 depend on this.
2. **Prediction accuracy reporting.** The `predict=X/Y` field in `ExhaustReport` blends structural-fact accuracy (100%) with ordering-heuristic accuracy (73%). These should be reported separately — different failure modes, different severity.
3. **No `.slow` budget tests.** Steps 4 and 5 activate only at `.slow` budget (≥ 3 stall cycles). The FibreDescentGate test uses `.slow` but other tests run at default `.fast`.
4. **Non-monotonicity detection** (Signal 7). Implemented. `BinarySearchEncoder` emits `nonMonotoneGap` when the remaining range is within the scan threshold. `LinearScanEncoder` resolves the question in ≤ 64 probes per coordinate. Net -65 materializations across the suite. See `docs/convergence-signals-design.md` for the full design.
5. **Adaptive scheduler heuristic gaps.** Two heuristics in `AdaptiveStrategy` have theoretical gaps where Phase 2's value changes could invalidate a prior cycle's observation:
   - **Phase 1 behavioral gate**: if Phase 1 found zero structural acceptances in cycle N, the gate skips Phase 1 in cycle N+1. But Phase 2 changes values, and a deletion that failed in cycle N (property passed on the shorter sequence) might succeed in cycle N+1 (property now fails on the shorter sequence with reduced values). This requires a non-monotone property response to value reduction on shortened sequences — smaller remaining values making the property *harder* to satisfy. Unusual but not impossible.
   - **`exhaustedClean` skip for structurally constant edges**: the skip fires when the upstream value matches the observed value. But the lift reads the full sequence as prefix, including downstream values that Phase 2 changed. The fibre's structure is invariant (structurally constant), but the specific values differ. A prior exhaustive search that found no failure might miss one in a fibre with reduced values.
   - Neither gap has produced a missed improvement on any test in the suite. A periodic re-check (run Phase 1 every k-th skipped cycle, re-explore skipped edges every k-th cycle) would close both gaps at the cost of occasional unnecessary materializations. Worth investigating if a generator class emerges where the gaps manifest.
6. **`zeroingDependency` escalation threshold.** Phase 4 (relax-round) runs when any `zeroingDependency` signal is present, regardless of how many coordinates are affected. On generators with hundreds of coordinates, a single coupled coordinate triggers a full checkpoint/exploitation pipeline. A proportional threshold (`zeroingDependencyCount / totalCoordinates > threshold`) would make escalation proportional to the scope of the coupling. Not a concern on the current suite (generators are small).
7. **Coordinate-level Phase 2 budget adaptation.** Phase 2 treats all coordinates equally — every coordinate gets the same binary search budget. Coordinates already at their floor (convergence cache entry with bound at target) produce zero probes but still pay span extraction and encoder setup cost. The factory could skip converged coordinates entirely within Phase 2, finer-grained than the fibre descent gate (which skips all of Phase 2 or none). This is the "budget is misspent on setup" problem: the adaptive scheduler's Phase 1 skip addresses it for Phase 1, but Phase 2 has the same issue for individual converged coordinates.
8. **`exhaustedWithFailure` floor propagation.** Currently `exhaustedWithFailure` is a boolean — the downstream found a failure but the observation doesn't carry the floor. If the observation included the downstream floor (smallest failing configuration), the factory could warm-start the next cycle's downstream search. This is the path to activating convergence transfer (gap 1) — even without a search-based downstream, `FibreCoveringEncoder` could skip combinations below the prior floor.
