...
4. **Non-monotonicity detection** (Signal 7). The signal infrastructure (`ConvergenceSignal.nonMonotoneGap`, `LinearScanEncoder`, factory consumption) is implemented and tested. The detection heuristic is deferred — within a single binary search run, every rejection occurs below a known failure, making in-stepper detection ambiguous. See `docs/convergence-signals-design.md` "Deferred: Phase 1b" for candidate detection approaches (cross-cycle analysis, validation probe extension, post-convergence spot check).
