...
5. **Adaptive scheduler heuristic gaps.** Two heuristics in `AdaptiveStrategy` have theoretical gaps where value minimisation's changes could invalidate a prior cycle's observation:
   - **Structural minimisation behavioral gate — partially closed.** Value minimisation can reduce a value to zero, making a previously-failed deletion viable (the shortened sequence now fails the property). This manifested on Bound5: value minimisation reduced an element to 0, but structural minimisation was skipped, leaving a stray `0` in the counterexample. Fixed with a **deletion probe**: when structural minimisation is skipped, a lightweight structural pass (budget: 100) runs at the end of the cycle to catch deletions enabled by value changes. The probe closed the gap on Bound5 and reduced materialisation counts on BinaryHeap (705 → 697) and Bound5Path3 (485 → 467).
   - **`exhaustedClean` skip for structurally constant edges**: the skip fires when the upstream value matches the observed value. But the lift reads the full sequence as prefix, including downstream values that value minimisation changed. The fibre's structure is invariant (structurally constant), but the specific values differ. A prior exhaustive search that found no failure might miss one in a fibre with reduced values. No test in the suite exhibits this gap. A periodic re-exploration would close it at the cost of occasional unnecessary materializations.
